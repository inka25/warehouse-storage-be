...
func TestListBrands(t *testing.T) {
	...
	var tts = []struct {
		caseName     string
		expectations func()
		results      func(response *dtos.ListBrandsResponse, err error)
	}{
		{
			caseName: "db error",
			expectations: func() {
				mockMysql.EXPECT().ListBrands(ctx).Return(nil, errors.New("any"))
			},
			results: func(response *dtos.ListBrandsResponse, err error) {
				assert.NotNil(t, err)
		...
		},
		{
			...
			expectations: func() {
				mockMysql.EXPECT().ListBrands(ctx).Return(nil, errs.ErrNoResultFound)
			},
			results: func(response *dtos.ListBrandsResponse, err error) {
				assert.NotNil(t, err)
		...
		},
		{
			caseName: "success",
			expectations: func() {
				mockMysql.EXPECT().ListBrands(ctx).
	...
	for _, tt := range tts {
		...
		t.Log(tt.caseName)
		tt.expectations()
		tt.results(handler.ListBrands(ctx))
	}
}
