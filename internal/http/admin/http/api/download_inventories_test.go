...
func TestDownloadInventories(t *testing.T) {
	...
	tts := []struct {
		caseName    string
		handlerFunc func(ctx context.Context, p *dtos.DownloadInventoriesRequest) (*dtos.DownloadInventoriesResponse, error)
	...
	}{
		{
			...
			request: func() *http.Request {
				...
				req, _ := http.NewRequest(http.MethodGet, "/download/inventories?warehouse_id=1", nil)
				return req
			},
			handlerFunc: func(ctx context.Context, p *dtos.DownloadInventoriesRequest) (*dtos.DownloadInventoriesResponse, error) {
				return &dtos.DownloadInventoriesResponse{
		...
		{
			caseName: "when there is invalid warehouse id",
			request: func() *http.Request {

				req, _ := http.NewRequest(http.MethodGet, "/download/inventories", nil)
				return req
			},
			handlerFunc: func(ctx context.Context, p *dtos.DownloadInventoriesRequest) (*dtos.DownloadInventoriesResponse, error) {
				return nil, nil
	...
	for _, tt := range tts {
		...
		router.Handle("/download/inventories", DownloadInventories(tt.handlerFunc))
